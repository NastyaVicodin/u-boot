...
static int setup_mem_map(void)
{
	 ...
	 * for the guest: console, xenstore etc.
	 */
	xen_mem_map[0].virt = GUEST_MAGIC_BASE;
	xen_mem_map[0].phys = GUEST_MAGIC_BASE;
	xen_mem_map[0].size = GUEST_MAGIC_SIZE;
	xen_mem_map[0].attrs = (PTE_BLOCK_MEMTYPE(MT_NORMAL) |
	...
	for (i = 1; i < MAX_MEM_MAP_REGIONS; i++) {
